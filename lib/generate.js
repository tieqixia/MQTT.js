...
module.exports.connect = function(opts) {
  var opts = opts || {}
    , protocolId = opts.protocolId || defaultProtocolId
    , protocolVersion = opts.protocolVersion || defaultProtocolVersion
    , will = opts.will
    , clean = opts.clean
...
module.exports.publish = function(opts) {
  ...
  // Remaining length
  var retain_length = write_length(buffer, pos, length)
  pos += retain_length;
...
var gen_pubs = function(opts, type) {
  ...
  var buffer = new Buffer(4 + id.length)
    , pos = 0;
  ...
  // Length
  pos += write_length(buffer, pos, 2+id.length);
