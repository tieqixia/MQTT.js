...
module.exports.header = function(buf, packet) {
  ...
  if ((/reserved|pubrec|pubcomp|pubrel/).test(packet.cmd)) packet.cmd = null;
  if (packet.dup) packet.cmd = null;
  if (packet.qos > 1) packet.cmd = null;
...
module.exports.connect = function(buf, packet) {
  ...
  pos = len = protocol_and_len = topic_and_len = username_and_len = null;
  client_and_len = payload_and_len = password_and_len = flags = buf = null;
...
module.exports.publish = function(buf, packet) {
  ...
  /* Parse message ID */
  messageId_and_len = parse_string(buf, len, pos);
  packet.messageId = messageId_and_len[0];
  if(packet.messageId === null) return null;
  pos += messageId_and_len[1] + 2;
