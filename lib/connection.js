...
Connection.prototype.parse = function(buf) {
  ...
  while (pos < len) {
    ...
    if (len - pos < this.packet.length) {
      ...
      var that = this;
      if(that.packet.cmd == 'pingreq'){
       process.nextTick(function(){
         parse[that.packet.cmd](
         that.buffer.slice(pos, that.packet.length + pos), that.packet);
         // Indicate that we've read all the data
         pos += that.packet.length;
         // Emit packet and reset connection state
         that.emit(that.packet.cmd, that.packet);
         that.packet = {};
       });
      }
      else{
